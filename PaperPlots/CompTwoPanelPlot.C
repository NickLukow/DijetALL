...
double YMAX = 0.125;  // with 9 0.125   without 0.105
...
if (PLOT9 == 1 && PLOTGLOBAL==1) LEGLOWY = 0.5;
  ...
  double EWrun9sysy[7] = {0.0005,0.0006,0.0007,0.0009,0.0013,0.0022,0.0036};
	...
	double EWrun15x[7] = {18.87, 22.03, 26.88, 32.76, 39.76, 49.69, 68.23};
  ...
  double EWrun15sysx[7] = {0.61, 0.59, 0.69, 0.80, 0.92, 1.08, 1.28};
  double EWrun15sysy[7] = {4.25E-04, 3.86E-04, 4.62E-04, 5.85E-04, 9.21E-04, 8.52E-04, 1.68E-03};
    
	double EEWWrun15x[7] = {18.74, 21.96, 26.77, 32.68, 39.51, 49.34, 67.62};
	...
	double EEWWrun15sysx[7] = {0.52, 0.61, 0.67, 0.77, 0.89, 1.02, 1.30};
...
//---------------> Theory
  ...
  TGraph *ALLdssvEEWW = new TGraph("./Theory/DSSV/Coarse_Trunc/EEWW_x.DSSV.txt", "%lg %lg");
  ALLdssvEEWW->SetLineColor(kRed);
  ALLdssvEEWW->SetLineStyle(7);
  ALLdssvEEWW->SetLineWidth(3);

  TGraph *ALLdssvEW = new TGraph("./Theory/DSSV/Coarse_Trunc/EW_x.DSSV.txt", "%lg %lg");
...
/*
  ...
  */
  ...
  if(PLOTGLOBAL) ALLdssvEW->Draw("csame");
  ...
  //leg2->AddEntry(bxLumi15, "Lumi. Syst.", "f");    
	if(PLOTGLOBAL) leg2->AddEntry(ALLdssvEW, "DSSV'14", "l");
  ...
  if(PLOTGLOBAL) ALLdssvEEWW->Draw("csame");
